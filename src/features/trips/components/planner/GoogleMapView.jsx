...
export default function GoogleMapView({ center, userPlaces = [], tripInfo }) {
    ...
    (async () => {
      try {
        ...
        userPlaces.forEach((p) => {
          ...
          Object.assign(iconEl.style, { fontSize: "18px", color: "#27ae60" });
          ...
          marker.addListener("click", () => {
            if (!isCancelled) setSelected(p);
          });

          markersRef.current.push(marker);
        });
      } catch (e) {
        console.error("Marker loading error", e);
      }
    })();
  ...
  return (
    ...
    >
      ...
      {!userPlaces.length && <Marker position={center} />}
    </GoogleMap>
  );
}
